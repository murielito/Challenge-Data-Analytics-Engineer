...
# Aquí debes usar el nombre correcto: 'std_prod_price'
#df_sellers['std_prod_price'] = df_sellers['std_prod_price'].fillna(0)
...
df_sellers['texto_seller'] = (
    "avg_prod_price: " + df_sellers['avg_prod_price'].round(2).astype(str) +
    ", max_prod_price: " + df_sellers['stock_median'].round(0).astype(int).astype(str) +
    ", unique_cat: " + df_sellers['unique_cat'].astype(str) +
    ", reputation_score: " + df_sellers['reputation_score'].round(2).astype(str) +
    ", condition_new_ratio: " + (df_sellers['condition_new_ratio'] * 100).round(0).astype(int).astype(str) +
    ", has_discount_ratio: " + (df_sellers['has_discount_ratio'] * 100).round(0).astype(int).astype(str)
)
...
reg_logistica = LogisticRegression(class_weight='balanced',max_iter=1000, multi_class='multinomial', solver='lbfgs',random_state=42)
...
nuevo_texto = (
    ...
    "su reputación es 8/10, "
    "tiene una ratio de productos nuevos de 0.05, "
    "y aplica descuentos en el 25% de sus productos (estrategia moderada)."
)

nuevo_vector = model.encode([nuevo_texto])
prediccion_nuevo = reg_logistica.predict(nuevo_vector)
print("Este nuevo vendedor pertenece al clúster:", prediccion_nuevo[0])

##El modelo tiene una probabilidad del 97.2% de asignar al nuevo vendedor al cluster 1 por su comportamiento comercial
proba = reg_logistica.predict_proba(nuevo_vector)
print("Probabilidades por clúster:", proba)
